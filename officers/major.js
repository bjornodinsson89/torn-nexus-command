...
class Major {
    ...
    applyStyles() {
        ...
        style.textContent = `
            ...
            * { box-sizing: border-box; }
            ...
            /* DRAWER */
            #nexus-drawer {
                position: fixed; top: 0; width: 420px; height: 100vh;
                background: var(--c-bg);
            ...
            /* HEADER */
            .drawer-header {
                padding: 15px; background: linear-gradient(90deg, rgba(0,243,255,0.15), transparent);
            ...
            .control-btn:hover { color: var(--c-alert); }

            /* TABS */
            #nexus-tabs { display: flex; flex-wrap: wrap; background: #000; border-bottom: 1px solid #333; }
            .nexus-tab {
                flex: 1; background: transparent; border: none; color: #666; padding: 12px 2px;
            ...
            .tile h3 {
                ...
                color: #888; border-bottom: 1px solid #222; padding-bottom: 4px;
            }
            ...
            .nexus-table tr:hover td { background: rgba(0, 243, 255, 0.05); color: #fff; }
            ...
            .act-att:hover { color: var(--c-alert); }
            .act-spy:hover { color: var(--c-neon); }
            ...
            .scanner-line {
                position: absolute; width: 100%; height: 2px; background: var(--c-neon);
    ...
    renderActivePanel() {
        ...
        if(target) {
            ...
            // Trigger specific renders based on stored data
        ...
        p.innerHTML = `
            <div class="tile">
                ...
                <div style="margin-top:10px; font-size:12px; color:#888; text-align:center;">
                    STATUS: ${this.renderStatusBadge(u.status)}
                </div>
            </div>
            ...
            <div class="tile">
                <h3>THREAT ASSESSMENT</h3>
                 <div style="text-align:center; padding:5px;">
                    <span style="font-size:20px; font-weight:bold; color:${u.threat>50? 'var(--c-alert)' : 'var(--c-neon)'}">${u.threat}%</span>
                </div>
                <div style="width:100%; background:#222; height:4px; margin-top:5px;">
                    <div style="width:${u.threat}%; background:${u.threat>50?'var(--c-alert)':'var(--c-neon)'}; height:100%;"></div>
                </div>
            </div>

            <div class="tile">
                <h3>GLOBAL ACTIVITY</h3>
                <canvas id="heatmap-cvs" width="350" height="60" style="width:100%; height:60px; image-rendering:pixelated;"></canvas>
            </div>
        `;
    ...
    renderFaction() {
        ...
        const rows = members.map(m => `
            <tr>
                ...
                <td style="color:#fff; font-weight:600;">${this.sanitize(m.name)}</td>
                <td>${m.level}</td>
                ...
                <table class="nexus-table">
                    <thead>
                        <tr>
                            ...
                            <th data-sort="status">STS</th>
                            <th data-sort="days">DAY</th>
                        </tr>
                    </thead>
                    <tbody>${rows}</tbody>
                </table>
    ...
    renderEnemies() {
        ...
        if(list.length === 0) {
            p.innerHTML = `<div class="loader-text">NO HOSTILES DETECTED</div>`;
            return;
        }
        ...
        const rows = list.map(m => `
            <tr>
                <td>${this.renderOnlineDot(m.onlineState)}</td>
                <td style="color:#fff">${this.sanitize(m.name)}</td>
                <td>${m.level}</td>
                <td>${this.renderStatusBadge(m.status)}</td>
                <td style="text-align:right">
                    <span class="act-btn act-att" data-id="${m.id}">⚔</span>
                    <span class="act-btn act-spy" data-id="${m.id}">👁</span>
                </td>
            </tr>
        `).join("");
        ...
        p.innerHTML = `
            <div class="tile">
                ...
                <table class="nexus-table">
                    <thead><tr><th>●</th><th>NAME</th><th>LVL</th><th>STS</th><th style="text-align:right">ACT</th></tr></thead>
                    <tbody>${rows}</tbody>
                </table>
            </div>
        `;
    ...
    renderChain() {
        ...
        if(!c) {
            p.innerHTML = `<div class="loader-text">CHAIN INACTIVE</div>`;
            return;
        }
        ...
        p.innerHTML = `
            <div class="tile" style="border-color:${isCrit ? 'var(--c-alert)' : '#333'}">
                <h3>CHAIN STATUS</h3>
                <div class="grid-2">
                    <div class="stat-box"><div class="stat-val" style="color:var(--c-neon)">${c.hits}</div><div class="stat-lbl">COUNT</div></div>
                    <div class="stat-box"><div class="stat-val" style="color:${isCrit?'var(--c-alert)':'#fff'}">${time}s</div><div class="stat-lbl">TIMEOUT</div></div>
                </div>
                ${isCrit ? `<div style="background:var(--c-alert); color:#000; text-align:center; font-weight:bold; margin-top:5px; animation:pulse 0.5s infinite">TIMEOUT IMMINENT</div>` : ''}
            </div>
            
            <div class="tile">
                 <h3>RECENT HITS</h3>
                 <div id="chain-log" style="font-size:10px; color:#888;"></div>
            </div>
        `;

        // Render Log
        if(c.log && c.log.length) {
            const logHTML = c.log.slice(0, 10).map(l => 
                `<div style="border-bottom:1px solid #222; padding:2px;">
                    <span style="color:#fff">${l.player}</span> 
                    hit for <span style="color:var(--c-neon)">${l.respect}</span>
                </div>`
            ).join("");
    ...
    renderTargets() {
        ...
        if(!list.length) {
            container.innerHTML = `<div class="loader-text">NO TARGETS</div>`;
            return;
        }
        ...
        const rows = list.map(t => `
            <tr>
                ...
                <td style="color:#fff">${this.sanitize(t.name)}</td>
                <td>${t.level}</td>
                <td>${this.renderStatusBadge(t.status)}</td>
                <td style="text-align:right">
                     <span class="act-btn act-att" data-id="${t.id}">⚔</span>
                </td>
            </tr>
        `).join("");
        ...
        container.innerHTML = `
            <table class="nexus-table">
                <thead><tr><th>●</th><th>NAME</th><th>LVL</th><th>STS</th><th>ACT</th></tr></thead>
                <tbody>${rows}</tbody>
            </table>
        `;
        ...
        p.innerHTML = `
            <div class="tile">
                ...
                <div class="chat-win" id="col-msgs">
                    <div class="msg msg-ai">[AI] Systems online. Ready for query.</div>
                </div>
                <input type="text" id="col-input" class="chat-input" placeholder="Enter command...">
            </div>
        `;
    ...
    buildSettingsPanel() {
        ...
        p.innerHTML = `
            <div class="tile">
                <h3>CONFIG</h3>
                <label style="color:#ccc; display:block;">Dock Side
    ...
    startSitrepRouter() {
        ...
        this.general.signals.listen("SITREP_UPDATE", d => {
            ...
            // Only re-render if that tab is looking at the data
        ...
        this.general.signals.listen("GLOBAL_SITREP_READY", d => {
            if(d.heatmap) {
                this.store.heatmap = d.heatmap;
    ...
    renderStatusBadge(st) {
        ...
        if(s.includes("hosp")) return `<span class="badge badge-hos">HOSP</span>`;
        if(s.includes("jail")) return `<span class="badge badge-hos">JAIL</span>`;
        if(s.includes("travel")) return `<span class="badge badge-travel">FLY</span>`;
    ...
    drawHeatmap(canvas, data) {
        ...
        data.forEach((val, i) => {
            const hPct = val / max;
            ctx.fillStyle = `rgba(0, 243, 255, ${Math.max(0.1, hPct)})`;
    ...
    attachActionListeners(root) {
        ...
        root.querySelectorAll(".act-spy").forEach(b => {
    ...
    attachDragLogic() {
        ...
        const start = (e) => {
            ...
            this.dragOffsetX = t.clientX - r.left; this.dragOffsetY = t.clientY - r.top;
            const move = (ev) => {
            ...
            const stop = () => {
                this.dragging = false; setTimeout(() => this._isDragging = false, 100);
    ...
    attachButtonLogic() {
        this.buttonEl.addEventListener("click", () => { if(!this._isDragging) this.toggleDrawer(); });
    }
    attachResizeObserver() { window.addEventListener("resize", () => this.updateDrawerSide()); }

    startInlineScanner() {
        // Basic scanner for profile links
        const scan = () => {
             document.querySelectorAll("a[href*='profiles.php?XID=']").forEach(a => {
                ...
                if(id) {
                    ...
                    s.innerHTML = `<span class="act-btn act-att" data-id="${id}" style="font-size:10px; margin-left:4px;">⚔</span>`;
                    a.after(s);
