...
// FIXES APPLIED:
//   - Removed ensureChart script injection (CSP fix)
//   - UI handles timeout/timeLeft correctly
//   - Guard against double-wrapping renderActiveTab override
...
const Major = {
    ...
    /* PATCH: Prevent recursive double-wrap */
...
Major.init = function(nexus){
    ...
    /* PATCH: Guard prevent multiple override */
    if (!this._renderWrapped){
        this._renderWrapped = true;
        const oldRender = this.renderActiveTab;
...
/* BLOCK: CHART.JS LOADER — REMOVED INJECTION */
/* PATCH: No injection — just check if Chart exists */
...
Major.ensureChart = function(cb){
    ...
    /* If Chart does not exist yet, wait until it loads via @require */
    const check = () => {
        if (window.Chart){
            cb();
        } else {
